...
####################################################################################################################################
# BANDITS Test Pi:
load("BANDITS results testPi.RData")
# load("BANDITS results Filtered testPi.RData") # for 6 vs 6 filtered transcriptome
res = top_transcripts(results)
fdr = res$adj.p.values
fdr_maxGene = res$Max_Gene_Tr.Adj.p.val
tr_id = res$Transcript_id
...
RES$BANDITS_testPi = fdr[match]
RES$BANDITS_testPi_maxGene = fdr_maxGene[match]
...
# name testPi_Tr3in1218LegendBottom, alias Figure S26.
