...
rm(fdr); rm(fdr_maxGene); rm(tr_id); rm(match); rm(results)
...
RES$BANDITS_maxGene_Filtered = fdr_maxGene[match]
...
# points only:
ggp_tr_BANDITS = plot_fdrtprcurve(cobraplot, linewidth = 1, pointsize = 5, plottype = "points",
                                  xaxisrange = c(0, 0.2),
                                  yaxisrange = c(0.65, 0.725)) +
...
ggp_tr_BANDITS_3vs3 = ggp_tr_BANDITS      # for 3 vs 3 simulation;
ggp_tr_BANDITS_6vs6 = ggp_tr_BANDITS      # for 6 vs 6 simulation;

####################################################################################################################################
# repeat the above code for
# 1) 3 vs 3 simulation (ggp_tr_BANDITS_3vs3)
# 2) 6 vs 6 simulation; (ggp_tr_BANDITS_6vs6)
# Then merge plots into 1 Figure as follows:

# tr PLOT:
ggarrange(ggp_tr_BANDITS_3vs3, ggp_tr_BANDITS_6vs6,
          labels = c("A", "B"),
          common.legend = TRUE, legend = "right",
          nrow = 2, ncol = 1)
# saved with size: 12 * 18 cm
# name 2in1BANDITSTr1218, alias Figure S4.
