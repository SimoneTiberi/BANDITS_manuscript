...
####################################################################################################################################
# BANDITS kallisto:
load("BANDITS kallisto results.RData")
# load("BANDITS kallisto results Filtered.RData") # for 6 vs 6 filtered transcriptome
res = top_genes(results)
fdr = res$adj.p.values
fdr_inv = res$adj.p.values_inverted
gene_id = res$Gene_id
...
RES$BANDITS_Salmon = fdr[match]
RES$BANDITS_Salmon_inv = fdr_inv[match]

rm(res); rm(fdr); rm(gene_id); rm(match); rm(results)

####################################################################################################################################
# BANDITS Salmon:
load("BANDITS Salmon results.RData")
# load("BANDITS Salmon results Filtered.RData") # for 6 vs 6 filtered transcriptome
res = top_genes(results)
fdr = res$adj.p.values
fdr_inv = res$adj.p.values_inverted
gene_id = res$Gene_id
...
RES$BANDITS_kallisto = fdr[match]
RES$BANDITS_kallisto_inv = fdr_inv[match]
...
ggp_gene = plot_fdrtprcurve(cobraplot, linewidth = 1, pointsize = 5, plottype = "points") +
  ...
  theme(legend.position="bottom", 
        ...
        aspect.ratio = 0.75) +
  xlim(0, 0.15) + ylim(0.8, 0.88) +
  guides(linetype = guide_legend(ncol = 3, byrow = FALSE))

ggp_gene_6vs6 = ggp_gene      # for 3 vs 3 vs 6 simulation;
ggp_gene_6vs6_filt = ggp_gene # for 3 vs 3 vs 6 simulation with transcript pre-filtering;

####################################################################################################################################
# repeat the above code for
# 1) 3 vs 3 vs 6 simulation; 3) 3 vs 3 vs 6 simulation (filtering transcripts).
# Then merge plots into 1 Figure as follows:
library(ggpubr)

ggarrange(ggp_gene_6vs6, ggp_gene_6vs6_filt,
          labels = c("A", "B"),
          common.legend = TRUE, legend = "right",
          nrow = 2, ncol = 1)
# saved with size: 12 * 18 cm
# name SalmonKallisto_Gene2in1218LegendBottom, alias Figure S23.
